...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.AFPLK == 'MJBOT' || config.AFPLK == 'mjbot' || config.AFPLK == 'Mjbot' || config.AFPLK == 'MjBot') {
        //thanks to afnanplk
        console.log(
            chalk.green.bold('thanks for watching -key cofirmed-')
        );
         }
         else if (config.AFPLK !== 'MJBOT' || config.AFPLK !== 'mjbot' || config.AFPLK !== 'Mjbot' || config.AFPLK !== 'MjBot') {
         console.log(
        ...
        console.log(
            chalk.green.bold('VavaBot Working ' + config.WORKTYPE + ' 𝚗𝚘𝚠 🍃'));
